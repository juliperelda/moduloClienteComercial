...
const Capacidad = () => {
  ...
  const [selectedValue, setSelectedValue] = useState(cosechaActiva);
  ...
  if (infoCap.length > 0) {
    console.log("infoCap desde Capacidad: ", infoCap);
    console.log("infoCap[0] desde Capacidad: ", infoCap[0].condicion);
    setSelectedValue(infoCap[0].acos_desc);
  }
  ...
  if (infoCosechas.length > 0) {
    ...
    var cosechaActiva = parseInt(localStorage.setItem("cosechaActiva",infoCosechas[0].acos_desc));
  }
  ...
  return (
    <>
      <div className="divDropdown">
        ...
        >
          {infoCosechas.length > 0 &&
            infoCosechas.map((cosecha) => {
              return (
                <Select.Option
